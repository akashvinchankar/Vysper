  ...
  class ChatWindowUI {
    ...
    init() {
      try {
        ...
        this.addMessage(
          "Chat window initialized. Click microphone or press ⌘+R to start recording.",
          "system"
        );
      ...
      if (window.electronAPI) {
        ...
        window.electronAPI.onTranscriptionReceived((event, data) => {
          if (data && data.text) {
            ...
            console.warn(
              "Transcription event received but no text data:",
              data
            );
          }
        });
        ...
        window.electronAPI.onSpeechStatus((event, data) => {
          if (data && data.status) {
            ...
            if (
              data.status.includes("started") ||
              data.status.includes("Recording")
            ) {
              this.handleRecordingStarted();
            } else if (
              data.status.includes("stopped") ||
              data.status.includes("ended")
            ) {
    ...
    setupUIHandlers() {
      ...
      this.elements.micButton.addEventListener("click", async () => {
        if (!this.isInteractive) {
          this.addMessage(
            "Window is in non-interactive mode. Press Alt+A to enable interaction.",
            "error"
          );
          return;
        }
    ...
    handleTranscription(text) {
      if (text && text.trim()) {
        ...
        logger.debug("Transcription received in chat", {
    ...
    async handleSkillActivated(skillName) {
      try {
        ...
        if (skillPrompt) {
          ...
          this.addMessage(
            `🎯 ${skillName.toUpperCase()} Mode: Ready to help!`,
            "system"
          );
        }
      } catch (error) {
        logger.error("Failed to load skill prompt", {
          skill: skillName,
          error: error.message,
        });
        // Fallback message
    ...
    async sendMessage() {
      ...
      if (text) {
        ...
        try {
          ...
          logger.error("Failed to send chat message to main process", {
            error: error.message,
          });
        }
      ...
      try {
        ...
        // Then try require (Node.js context)
          ...
          try {
            ...
            logger.debug(
              "Could not require markdown library:",
        ...
        if (markdownLib && markdownLib.toHTML) {
          ...
          logger.warn(
            "Markdown library not available, falling back to basic formatting"
          );
        ...
        logger.warn("Failed to parse markdown, falling back to plain text", {
          error: error.message,
        });
        // Fallback to basic formatting
        return text.replace(/\n/g, "<br>");
    ...
    showInteractionIndicator(text, interactive) {
      if (!this.elements.interactionIndicator || !this.elements.interactionText)
        return;

      this.elements.interactionText.textContent = text;
      this.elements.interactionIndicator.className = `interaction-indicator show ${
    ...
    showInterimText(text) {
      ...
      let interimElement =
        this.elements.listeningContainer.querySelector(".interim-text");
      if (!interimElement) {
