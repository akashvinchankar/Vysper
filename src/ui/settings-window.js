document.addEventListener("DOMContentLoaded", () => {
  ...
  const dockVisibilityItem = document.getElementById("dockVisibilityItem");
  const hideDockIconToggle = document.getElementById("hideDockIcon");
  ...
  const loadSettingsIntoUI = (settings) => {
    if (settings.azureKey && azureKeyInput)
      azureKeyInput.value = settings.azureKey;
    if (settings.azureRegion && azureRegionInput)
      azureRegionInput.value = settings.azureRegion;
    if (settings.geminiKey && geminiKeyInput)
      geminiKeyInput.value = settings.geminiKey;
    if (settings.windowGap && windowGapInput)
      windowGapInput.value = settings.windowGap;
    if (settings.codingLanguage && codingLanguageSelect)
    ...
    if (
      dockVisibilityItem &&
      window.electronAPI &&
      window.electronAPI.getDockVisibilityStatus
    ) {
      window.electronAPI
        .getDockVisibilityStatus()
        .then((status) => {
          if (status && status.supported) {
            dockVisibilityItem.style.display = "flex";
            if (hideDockIconToggle) {
              // Prefer live status; fallback to saved setting
              hideDockIconToggle.checked = Boolean(
                status.hidden ?? settings.hideDockIcon
              );
            }
          }
        })
        .catch((err) => console.error("Dock status check failed:", err));
    }
  ...
  // Dock visibility toggle (macOS)
  if (
    hideDockIconToggle &&
    window.electronAPI &&
    window.electronAPI.toggleDockVisibility
  ) {
    hideDockIconToggle.addEventListener("change", async (e) => {
      try {
        const hide = e.target.checked;
        await window.electronAPI.toggleDockVisibility(hide);
        // Save preference
        window.api.send("save-settings", { hideDockIcon: hide });
      } catch (error) {
    ...
    const icons = [
      {
        key: "terminal",
        name: "Terminal",
        src: "./assests/icons/terminal.png",
      },
      {
        key: "activity",
        name: "Activity",
        src: "./assests/icons/activity.png",
      },
      {
        key: "settings",
        name: "Settings",
        src: "./assests/icons/settings.png",
      },
    ];
