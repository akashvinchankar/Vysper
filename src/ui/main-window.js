...
class MainWindowUI {
  ...
  async loadCurrentInteractionState() {
    try {
      ...
      this.isInteractive = false;
      logger.warn(
        "Failed to load current interaction state, defaulting to non-interactive",
        {
          component: "MainWindowUI",
          error: error.message,
        }
    ...
    if (window.api) {
      ...
      window.api.receive("skill-updated", (data) => {
        ...
        if (data && data.skill) {
          ...
          logger.warn(
            "Skill updated event received but no skill data found:",
            data
          );
        }
      });
  ...
  handleLLMResponse(data) {
    ...
    const skillNames = {
      ...
      reactjs: "React.js",
      "react-machine-coding": "React MC",
      "frontend-interview": "Frontend Q&A",
  ...
  handleInteractionModeChanged(interactive) {
    ...
    logger.info("Interaction mode change completed", {
      ...
      skillIndicatorClass: this.skillIndicator
        ? this.skillIndicator.className
        : "not found",
    });
  }
  ...
  updateSkillIndicator() {
    ...
    const skillName =
      skillNames[this.currentSkill] || this.currentSkill.toUpperCase();
  ...
  moveWindow(direction) {
    ...
    if (window.electronAPI && window.electronAPI.moveWindow) {
      ...
      logger.warn("moveWindow API not available", {
        component: "MainWindowUI",
      });
    }
  }
    ...
    modal.innerHTML = `
            <div class="bg-gray-900 text-white p-6 rounded-lg max-w-md w-full">
                ...
                <div class="mb-4 p-3 rounded ${
                  status.hasApiKey ? "bg-green-900" : "bg-red-900"
                }">
  ...
  showSettingsMenu() {
    ...
    const quitOption = this.createMenuItem(
      "Quit Vysper",
      "fa-power-off",
      () => {
        if (window.electronAPI) {
    ...
    const closeMenu = (e) => {
      if (
        ...
        !this.settingsIndicator.contains(e.target)
      ) {
        document.body.removeChild(menu);
...
if (typeof document !== "undefined") {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    logger.info(
      "MainWindowUI initialized and available as window.mainWindowUI"
    );
  });
}
