...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  enableWindowInteraction: () =>
    ipcRenderer.invoke("enable-window-interaction"),
  disableWindowInteraction: () =>
    ipcRenderer.invoke("disable-window-interaction"),
  ...
  getSkillPrompt: (skillName) =>
    ipcRenderer.invoke("get-skill-prompt", skillName),
  ...
  updateActiveSkill: (skill) =>
    ipcRenderer.invoke("update-active-skill", skill),
  restartAppForStealth: () => ipcRenderer.invoke("restart-app-for-stealth"),
  // Dock visibility (macOS)
  toggleDockVisibility: (hide = null) =>
    ipcRenderer.invoke("toggle-dock-visibility", hide),
  getDockVisibilityStatus: () =>
    ipcRenderer.invoke("get-dock-visibility-status"),
  hideDockIcon: () => ipcRenderer.invoke("hide-dock-icon"),
  showDockIcon: () => ipcRenderer.invoke("show-dock-icon"),
  ...
  expandLlmWindow: (contentMetrics) =>
    ipcRenderer.invoke("expand-llm-window", contentMetrics),
  resizeLlmWindowForContent: (contentMetrics) =>
    ipcRenderer.invoke("resize-llm-window-for-content", contentMetrics),

  // Event listeners
  onTranscriptionReceived: (callback) =>
    ipcRenderer.on("transcription-received", callback),
  onInterimTranscription: (callback) =>
    ipcRenderer.on("interim-transcription", callback),
  ...
  onTranscriptionLlmResponse: (callback) =>
    ipcRenderer.on("transcription-llm-response", callback),
  onOpenGeminiConfig: (callback) =>
    ipcRenderer.on("open-gemini-config", callback),
  onDisplayLlmResponse: (callback) =>
    ipcRenderer.on("display-llm-response", callback),
  ...
  onInteractionModeChanged: (callback) =>
    ipcRenderer.on("interaction-mode-changed", callback),
